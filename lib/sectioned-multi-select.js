...
import {
  ActivityIndicator, FlatList, Modal, Platform, SafeAreaView, StyleSheet, Text, TextInput, TouchableNativeFeedback, TouchableOpacity, TouchableWithoutFeedback, View
} from 'react-native'
import { RowItem } from './components'
import { callIfFunction, callWithIfFunction } from './helpers'
...
const defaultTextInput = props => <TextInput {...props} />
...
class SectionedMultiSelect extends PureComponent {
  static propTypes = {
    ...
    searchTextInputComponent: PropTypes.oneOfType([PropTypes.func, PropTypes.object]),
    ...
    itemEndOfLineComponent: PropTypes.oneOfType([PropTypes.func, PropTypes.object]),
  ...
  static defaultProps = {
    ...
    searchTextInputComponent: defaultTextInput,
    loading: false,
    styles: {},
    colors: {},
    itemFontFamily: {
  ...
  _setSearchTerm = (term = '') => {
    ...
    callWithIfFunction(this.props.onSearchValueChange, term)
  }
  _itemSelected = (item) => {
    ...
    return (
      <View>
        <RowItem
          ...
          _setSearchTerm={this._setSearchTerm}
          {...this.props}
        />
      </View>
    )
  ...
  _renderSearchInput = () => {
    const { colors, styles } = this.state
    const { searchTextInputComponent, autoFocus, searchPlaceholderText, searchTextFontFamily } = this.props
    ...
    return searchTextInputComponent({
      ...
      onChangeText: searchTerm => this._setSearchTerm(searchTerm),
      placeholder: searchPlaceholderText,
      autoFocus,
      onSetSearchValue: this._setSearchTerm,
      ...
      <View>
        ...
        >
          ...
          >
            ...
            {!hideSearch && (
              <View keyboardShouldPersistTaps="always" style={[{ flexDirection: 'row', paddingVertical: 5 }, styles.searchBar]}>
                <View style={styles.center}>
                  {callIfFunction(searchIconComponent) || (
                    <this.Icon name="search" size={18} style={{ marginHorizontal: 12 }} />
                  )}
                </View>
